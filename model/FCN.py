...
import numpy as np
from model.backbones.mobilenetv3 import mobilenetv3_large
from model.backbones.mobilenetv2 import mobilenetv2
from utils.backend_layers import upsampling_layer, conv2d
    ...
    with tf.variable_scope('FCN') as scope:
        with tf.variable_scope('encoder') as scope:
            ...
            if backbone_type=='mobilenetv2':
                feature_4s, feature_8s, feature_16s, feature_32s = mobilenetv2(input=input, is_training=is_training, input_size=input_size)

        with tf.variable_scope('decoder') as scope:
            scores_32s = conv2d(name='scores_32s', x=feature_32s, filter_shape=1, input_channels=feature_32s.shape[-1],
                                output_channels=num_classes, strides=(1, 1), nl=None, padding='SAME', use_bn=False,
                                use_bias=True, activation=False, is_training=True)
            ...
            scores_16s = conv2d(name='scores_16s', x=feature_16s, filter_shape=1, input_channels=feature_16s.shape[-1],
                                output_channels=num_classes, strides=(1, 1), nl=None, padding='SAME', use_bn=False,
                                use_bias=True, activation=False, is_training=True)
            ...
            up_16s_to_8s = upsampling_layer(name='16to8', x=logits_16s, channels=num_classes, input_size=input_size//16,
                                            upsampling_type='bilinear', upsample_rate=2)
            scores_8s = conv2d(name='scores_8s', x=feature_8s, filter_shape=1, input_channels=feature_8s.shape[-1],
                                output_channels=num_classes, strides=(1, 1), nl=None, padding='SAME', use_bn=False,
                                use_bias=True, activation=False, is_training=True)
            ...
            up_8s_to_4s = upsampling_layer(name='8to4', x=logits_8s, channels=num_classes, input_size=input_size//8,
                                            upsampling_type='bilinear', upsample_rate=2)
            scores_4s = conv2d(name='scores_4s', x=feature_4s, filter_shape=1, input_channels=feature_4s.shape[-1],
                                output_channels=num_classes, strides=(1, 1), nl=None, padding='SAME', use_bn=False,
                                use_bias=True, activation=False, is_training=True)
