...
def mobilenetv2_16s(input, is_training, input_size=224, num_classes=3):
    with tf.variable_scope('encoder') as scope:
        with tf.variable_scope('mobilenetv2_16s') as scope:
            ...
            head_dwconv = depthwise_conv2d(name='head_dwconv', x=head_conv1, channels=32, filter_shape=3,
                                           strides=(1, 1, 1, 1),
    ...
    with tf.variable_scope('decoder') as scope:
        conv1 = conv2d(name='conv1', x=bottleneck6_1, filter_shape=1, input_channels=320, output_channels=1280,
                       strides=1, nl='relu6', padding='SAME', use_bn=False, use_bias=True, activation=True,
                       is_training=is_training)
        avg_pool = tf.nn.avg_pool2d(name='global_avgpool', value=conv1, ksize=input_size // 16, strides=[1, 1],
                                    padding="VALID")
        logits = conv2d(name='logits', x=avg_pool, filter_shape=1, input_channels=1280, output_channels=num_classes,
                        strides=1, nl=None, padding='SAME', use_bn=False, use_bias=True, activation=False,
                        is_training=is_training)
...
if __name__ == '__main__':
    ...
    with g1.as_default():
        ...
        x = mobilenetv2_16s(input=input, is_training=False)
        ...
        with tf.Session() as sess:
            ...
            with tf.gfile.FastGFile('../../model/visualization/mobilenetv2_16s.pb', mode='wb') as f:
