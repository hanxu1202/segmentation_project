...
def mobilenetv2(input, is_training, input_size=224, num_classes=3):
    with tf.variable_scope('encoder') as scope:
        with tf.variable_scope('mobilenetv2') as scope:
            ...
            bottleneck5_1 = v2Bottleneck(name='bottleneck5_1', x=bottleneck4_3, input_channels=96, expand_channels=576,
                                         output_channels=160, dwconv_ksize=3, is_ds=True, is_training=is_training)  # /32
    ...
    with tf.variable_scope('decoder') as scope:
        conv1 = conv2d(name='conv1', x=bottleneck6_1, filter_shape=1, input_channels=320, output_channels=1280, strides=1,
                       nl='relu6', padding='SAME', use_bn=False, use_bias=True, activation=True, is_training=is_training)
        avg_pool = tf.nn.avg_pool2d(name='global_avgpool', value=conv1, ksize=input_size // 32, strides=[1, 1],
                                    padding="VALID")
        logits = conv2d(name='logits', x=avg_pool, filter_shape=1, input_channels=1280, output_channels=num_classes, strides=1,
...
if __name__ == '__main__':
    ...
    with g1.as_default():
        ...
        with tf.Session() as sess:
            ...
            constant_graph = graph_util.convert_variables_to_constants(sess, sess.graph_def, ['decoder/logits/output'])
            with tf.gfile.FastGFile('../../model/visualization/mobilenetv2.pb', mode='wb') as f:
